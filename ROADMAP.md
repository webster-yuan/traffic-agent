...
**当前问题**: 桌面端使用 CSS Grid 双栏布局，移动端/窄屏下布局塌陷。
**状态**: ✅ 已修复（2026-04-29）— `@media (max-width: 768px)` 单栏 + form-grid 单列 + tab 按钮调小。

### 1.2 历史记录无分页 🔴 → ✅

**当前问题**: `refreshHistory()` 固定请求 `page=1, page_size=20`，只展示最近 20 条。
**状态**: ✅ 已修复（2026-04-29）— Store 加 `historyPage`/`historyTotalPages` + `goHistoryPage()`，UI 加 ←上一页/下一页→ 控件。
...
**当前问题**: 所有面板（生成、批量、历史）堆在一个页面，无标签页导航。
**状态**: ✅ 已修复（2026-04-29）— 顶部 Tab 导航栏，三个 Section 用 `v-if="activeTab"` 切换。

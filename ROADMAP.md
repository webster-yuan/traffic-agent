...
**更新日期**: 2026-04-26
**重点**: 已支持 JSON 与 CSV 双格式导出；下一优先项为质量评分解释与任务详情展示
...
1. 优先做 **质量评估解释性增强**，在任务详情中展示各维度评分和扣分原因（可与 JSON `metadata.quality` 字段对齐展示）。
2. 随后考虑 **Parquet/报表类导出** 或 **批量生成功能**，视数据量与使用场景再定。
3. **响应式布局优化** 按移动端使用频率排期。

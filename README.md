...
Traffic Agent 是一个全栈 AI 应用，结合 LangGraph、FastAPI 和 Vue 3，使用本地大语言模型（Ollama + Qwen2.5 7B）生成高质量的模拟网络流量数据。系统采用 **Supervisor-Worker 多智能体架构**（RAG → 生成 → 评估 → 审批 → 身份校验），支持 Send() 并行扇出、子图嵌套、Human-in-the-Loop 审批、SSE 流式思考链、ECharts 可视化报表、批量生成、7 维度历史筛选、CSV/JSON/Parquet 多格式导出。

> 📖 **Agent 技术深度文档**: 见 [docs/Agent核心技术落地.md](docs/Agent核心技术落地.md) — 涵盖 Supervisor-Worker 架构、结构化输出、并行扇出、子图嵌套、流式思考链、检查点持久化等核心技术落地要点。

## 核心特性

- **Supervisor-Worker 多智能体**: LangGraph StateGraph + 结构化输出 RouterDecision + Send() 并行扇出 + 子图嵌套 + Human-in-the-Loop 审批
- **AI 驱动**: Ollama + Qwen2.5 7B 本地模型，异步调用 + asyncio.wait_for 超时保护
- **RAG 增强**: 12 个行业静态示例 JSON 注入 LLM system prompt，提升生成质量 + 行业配置 API 按需加载
- **Agent 思考链**: SSE Custom Streaming 实时推送 Supervisor 决策 + 生成进度，前端滚动日志展示
- **质量评分**: 格式（30%）+ 业务（40%）+ 多样性（30%），Pandera 声明式 15 规则校验，未通过自动重试 + 自优化反馈
- **HITL 审批**: LangGraph `interrupt()` 动态中断，前端审批面板（统计摘要 + 样例记录），Approve/Reject → 恢复/重新生成
- **Checkpoint 回放**: `aget_state_history()` Time Travel，从任意节点状态 Fork 新会话回放
...
- **SSE 流式**: 实时推送五阶段进度（RAG/生成/评估/审批/身份校验），支持取消 + 错误重试
- **安全与可观测性**: slowapi 速率限制（200/day）+ Request ID 全链路 + LangSmith Trace + session_id/thread_id 关联
...
- **数据库**: SQLite (aiosqlite 异步 + AsyncSqliteSaver 检查点持久化)
- **验证**: Pydantic + Pandera 声明式 DataFrame Schema (15 校验规则)
- **图表**: ECharts (HTML 报表内嵌)
- **速率限制**: slowapi (200/day 全局限流) + RequestIDMiddleware
- **测试**: Pytest (71 个测试用例)

### 前端
- **框架**: Vue 3 + TypeScript + Vite 5
- **状态管理**: Pinia (trafficStore — 含 HITL 审批状态管理)
- **组件**: Composition API + 3 个 Panel 组件 (Generate / History / Batch)
- **测试**: Vitest (7 个测试用例)
...
| POST | /api/v1/traffic/generate/stream | 生成流量数据（SSE 流式，含审批中断） |
| DELETE | /api/v1/traffic/generate/{session_id} | 取消生成 |
| POST | /api/v1/traffic/resume/{session_id} | **HITL**: 恢复暂停的审批会话 |
...
| GET | /api/v1/traffic/checkpoints/{session_id} | **Time Travel**: 列出检查点（分页） |
| POST | /api/v1/traffic/replay | **Time Travel**: 从检查点回放 |
| POST | /api/v1/traffic/batch | 批量生成（最多 10 任务） |
| GET | /api/v1/traffic/batch/{batch_id} | 批量任务状态 |
| GET | /api/v1/traffic/industries | **行业配置**: 前端按需加载 12 行业 |
| GET | /health | 健康检查 |
...
├── backend/
│   ├── app/
│   │   ├── api/routes.py              # API 路由（生成/历史/下载/报表/批量/回放/行业）
│   │   ├── core/
│   │   │   ├── config.py              # 配置管理
│   │   │   ├── state.py               # 取消状态管理
│   │   │   ├── utils.py               # 通用工具（dedupe_notes 等）
│   │   │   └── json_utils.py          # JSON 修复工具
│   │   ├── data/industries.py         # 12 行业配置（单一数据源 + body_template 模板系统）
│   │   ├── db/database.py             # SQLite (aiosqlite 异步)
│   │   ├── graph/
│   │   │   ├── state.py               # GraphState 类型定义
│   │   │   ├── workflow.py            # LangGraph 工作流编译（含 approval 节点）
│   │   │   ├── supervisor.py          # Supervisor 决策节点 + 路由函数
│   │   │   ├── workers.py             # 5 个 Worker (rag/generate/eval/approval/identity)
│   │   │   ├── generate_subgraph.py   # Generate 子图 (prepare_prompt→call_llm→parse_result)
│   │   │   └── shared.py              # 共享辅助（_check_cancelled）
│   │   ├── models/schemas.py          # Pydantic 数据模型（Industry Literal 12 值）
│   │   ├── services/
│   │   │   ├── generator.py           # LLM 生成 + 质量评分 + CSV/JSON/Parquet 写入
│   │   │   ├── quality_validator.py   # Pandera 声明式校验 (15 规则)
│   │   │   ├── graph_runner.py        # 图执行器 (同步/异步/Checkpoint 回放)
│   │   │   ├── report_service.py      # HTML 报表生成 (ECharts 图表)
│   │   │   ├── session_service.py     # 会话 CRUD + 7 维度筛选 + 批量任务
│   │   │   ├── langchain_service.py   # LLM hint 构建
│   │   │   ├── llm_factory.py         # ChatOllama 工厂（单一实例化入口）
│   │   │   └── tracing_config.py      # LangSmith 追踪配置
│   │   ├── tools/                     # LangChain Tools (rag_search/quality_eval/identity_check/traffic_generate)
│   │   └── main.py                    # FastAPI 应用入口（含 slowapi + RequestID 中间件）
│   ├── tests/                         # 71 个 pytest 测试
│   ├── data/
...
│   │   ├── stores/                    # Pinia stores (trafficStore — 含 HITL 审批)
│   │   ├── App.vue                    # Tab 导航主框架
│   │   ├── main.ts
...
└── docs/
    ...
    ├── 开发设计文档_v1.0.md     ← 企业级开发设计文档 (v3.0)
    ├── ROADMAP.md              ← 路线图与下一步指引
    ├── SKILL_INVOCATION_GUIDE.md ← Skill 调用逻辑指南
    └── 流量字段枚举.md
```
...
5. 质量通过 + full 模式 → Approval (HITL interrupt)
6. Approval 通过 → Identity
7. 质量通过 + 非 full 模式 → END
8. 全部完成 → END
...
| 模式 | RAG | 评估 | 身份校验 | HITL 审批 | 默认数量 |
|------|-----|------|----------|-----------|----------|
| quick | 静态 JSON 示例 | - | - | - | 100 条 |
| standard | 静态 JSON 示例 | Pandera 15 规则 | - | - | 100 条 |
| full | 静态 JSON 示例 | Pandera 15 规则 | mock | interrupt() | 100 条 |
